// ==========================================================
// NEPPLAY — admin.js
// All admin panel logic — extracted from admin.html
// ==========================================================
...
// Attach live preview updates after DOM ready
...
window.submitCreateTournament = async function(e) {
  ...
  try {
    ...
    statusEl.innerHTML = `✅ Tournament "${title}" created successfully! It's now live on the member tournaments page.`;
...
window.loadRecentCreated = async function() {
  ...
  try {
    ...
    list.innerHTML = items.slice(0, 8).map(t => {
      ...
      return `
        <div class="reg-card ${isPaid ? 'paid-reg' : 'free-reg'}">
          ...
          <div class="reg-right"><span class="type-chip ${isPaid ? 'paid' : 'free'}">${isPaid ? 'Paid' : 'Free'}</span></div>
        </div>
      `;
    }).join('');
  } catch (err) {
...
(async function init() {
  ...
  console.log("✅ Admin loaded from admin.js");
})();
