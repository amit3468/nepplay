...
} from "https://www.gstatic.com/firebase// ==========================================================
// NEPPLAY — admin.js (v3.7)
// All admin panel logic
// Changes vs v3.6:
//   - Decode + display base64 payment screenshots (Spark plan)
//   - "Reset to Pending" button on approved/rejected payments
//   - "Re-approve" button on rejected payments
//   - Safer screenshot lightbox (no huge base64 in onclick attrs)
// ==========================================================
...
// Build the "src" attribute for a screenshot preview
function screenshotSrc(doc) {
  if (!doc) return null;
  // New: base64 stored inline
  if (doc.screenshotBase64) {
    return 'data:image/jpeg;base64,' + doc.screenshotBase64;
  }
...
// Register screenshot in cache so lightbox can find it without huge onclick strings
...
function renderPayments() {
  ...
  list.innerHTML = filtered.map(p => {
    ...
    // Register screenshot in cache, use doc ID for the lightbox
...
function renderRegistrations() {
  ...
  list.innerHTML = filtered.map(r => {
    ...
    // Find matching payment doc to get screenshot
...
window.confirmReview = async function(e) {
  ...
  if (action === 'approve') {
    status = 'approved'; regStatus = 'confirmed';
  } else if (action === 'reject') {
    status = 'rejected'; regStatus = 'rejected';
  } else if (action === 'reset') {
    status = 'pending'; regStatus = 'pending';
  } else {
    return;
  }
...
// Legacy support — still works if something passes a URL
...
})();js/10.12.0/firebase-firestore.js";
